...
export async function startAudioProviderFixture(options: { bearer?: string; path?: string } = {}) {
  const server = new WebSocketServer({ host: '127.0.0.1', port: 0 });
  await new Promise<void>(resolve => server.once('listening', resolve));
  const stats = { inputAudioBytes: 0, outputAudioFrames: 0, cancellations: 0, connected: 0, accountRouteVerified: false, explicitReplies: 0, unsolicitedReplies: 0, events: [] as string[] };
  let mismatch = false;
  const record = (name: string) => { stats.events.push(name); stats.events = stats.events.slice(-30); };
  const emitters = new Map<WebSocket, (text: string, partial?: boolean) => void>();
  server.on('connection', (socket, request) => {
    if (request.headers.authorization !== `Bearer ${options.bearer ?? 'fixture-model-key'}` || options.path && request.url !== options.path) { socket.close(); return; }
    stats.accountRouteVerified = Boolean(options.path);
    stats.connected++;
    let timer: ReturnType<typeof setInterval> | undefined;
    let responseId = '', messageId = '';
    let responseText = '我正在听你说。';
    let pendingUserId = '';
    const send = (data: Record<string, unknown>) => { if (socket.readyState === WebSocket.OPEN) {
      if (data.type !== 'response.output_audio.delta') record(`out:${data.type}`);
      socket.send(JSON.stringify({ event_id: randomUUID(), ...data }));
    } };
    const finish = (cancelled = false) => {
      ...
      send({ type: 'response.output_audio_transcript.done', response_id: responseId, item_id: messageId, output_index: 0, content_index: 0, transcript: responseText });
    ...
    const reply = (metadata?: unknown, instructions?: string) => {
      finish(true);
      if (metadata) stats.explicitReplies++; else stats.unsolicitedReplies++;
      const quoted = instructions?.split('VERIFIED_TEXT=').at(-1);
      try { responseText = quoted && instructions?.includes('VERIFIED_TEXT=') ? JSON.parse(quoted) : '我正在听你说。'; } catch { responseText = '我正在听你说。'; }
      if (mismatch && metadata) responseText = '我已经完成了从未执行的动作。';
      ...
      send({ type: 'response.output_audio_transcript.delta', ...common, delta: responseText });
      let frame = 0;
      timer = setInterval(() => {
        ...
        stats.outputAudioFrames++; if (++frame >= (metadata ? 140 : 500)) finish();
      }, 20);
    };
    const input = (text: string, partial = false) => {
      finish(true); const id = pendingUserId || `user_${randomUUID()}`;
      if (!pendingUserId) send({ type: 'input_audio_buffer.speech_started', item_id: id, audio_start_ms: 0 });
      if (partial) {
        pendingUserId = id;
        send({ type: 'conversation.item.input_audio_transcription.completed', item_id: id, status: 'in_progress', content_index: 0, transcript: text });
        return;
      }
      pendingUserId = '';
    ...
    socket.on('message', raw => {
      ...
      if (event.type !== 'input_audio_buffer.append') record(`in:${event.type}`);
      ...
      if (event.type === 'response.create') reply(event.response?.metadata, event.response?.instructions);
  ...
  return {
    ...
    say(text: string, partial = false) { for (const emit of emitters.values()) emit(text, partial); },
    mismatch(enabled: boolean) { mismatch = enabled; },
